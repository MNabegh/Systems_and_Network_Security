...
f_prime_permutation_table = [16, 7, 20, 21, 29, 12, 28, 17,
...
def f_encrypt(input):
    for i in range(16):
        ...
        R_n1 = [x ^ y for x, y in zip(L_n, R_n_dash)]
...
def f_prime(input, key):
    ...
    xor_output = [x ^ y for x, y in zip(expand_output, key)]
    combine_output = substitute(xor_output)
    result = permute_and_eliminate(combine_output, f_prime_permutation_table)
    return result


def substitute(input48):
    Bs = split_to_multiple_lists(input48, 6)  # splitting the 48 bit vector into 8 sublists
    combined_list = list()
    for i in range(len(Bs)):  # For all the sublists
        block = Bs[i]
        row = int(str(block[0]) + str(block[5]), 2)
        column = int(''.join([str(x) for x in block[1:][:-1]]), 2)
        val = s_tables[i][row][column]
        bin_output = bin(val)[2:]  # Convert the value to binary
        while len(bin_output) < 4:
            bin_output = "0" + bin_output
        combined_list += [int(x) for x in bin_output]  # And append it to the resulting list
...
def CBC(input, key):
    ...
    for i in range(0, len(inputInBinary), 64):
        if i == 0:
            firstIteration = xor(inputInBinary[i:i + 64], initial_vector)
            afterIP = permute_and_eliminate(firstIteration, IP)
            ciphered_message = f_encrypt(afterIP)
            swapped = swap_left_right(ciphered_message)
            c1 = permute_and_eliminate(swapped, IP_inverse)
            encrypted = c1
        else:
            nextIteration = xor(inputInBinary[i:i + 64], c1)
            afterIP = permute_and_eliminate(nextIteration, IP)
            ciphered_message = f_encrypt(afterIP)
            swapped = swap_left_right(ciphered_message)
            c1 = permute_and_eliminate(swapped, IP_inverse)
            encrypted += c1
